...
export async function getAvailableBots(
  ...
  try {
    ...
    logger.warn(
      `Failed to list chat bot members, skipping bot section: ${err}`
    );
    return [];
  }
}
...
export function buildNewTopicPrompt(
  ...
  if (availableBots && availableBots.length > 0) {
    const mentionedOpenIds = new Set(
      mentions?.map((m) => m.openId).filter(Boolean)
    );
    const unmentionedBots = availableBots.filter(
      (b) => !mentionedOpenIds.has(b.openId)
    );
    if (unmentionedBots.length > 0) {
      const botLines = unmentionedBots.map(
        (b) => `- ${b.displayName} (open_id: ${b.openId})`
      );
      botSection = `\n\n当前群聊中的其他机器人：\n${botLines.join(
        "\n"
  ...
  if (followUps && followUps.length > 0) {
    for (const fu of followUps) {
      parts.push(
        adapter.injectsSessionContext ? fu : `用户追加了：\n---\n${fu}\n---`
      );
    }
  }
...
export function persistStreamCardState(ds: DaemonSession): void {
  const cardId =
    ds.streamCardId === STREAM_CARD_SENTINEL ? undefined : ds.streamCardId;
  const s = ds.session;
...
export function restoreActiveSessions(
  ...
  logger.info(
    `Registering ${active.length} active session(s) (no CLI spawn until new messages arrive)...`
  );
  ...
  for (const session of active) {
    ...
    if (session.title?.startsWith("Adopt:") && session.adoptedFrom) {
      ...
      if (!validateAdoptTarget(adopted.tmuxTarget, adopted.originalCliPid)) {
        logger.info(
          `Closing adopt session ${session.sessionId} (original CLI exited)`
        );
      ...
      const larkAppId =
        session.larkAppId ?? getAllBots()[0]?.config.larkAppId ?? "";
      const ds: DaemonSession = {
        ...
        displayMode:
          session.displayMode ?? (session.streamExpanded ? "text" : "hidden"),
      ...
      logger.info(
        `[${session.sessionId.substring(
          0,
          8
        )}] Restored adopt session (target: ${adopted.tmuxTarget})`
      );
      continue;
    }
    ...
    if (session.title?.startsWith("Adopt:")) {
      logger.debug(
        `Closing adopt session ${session.sessionId} (no persisted metadata)`
      );
      sessionStore.closeSession(session.sessionId);
      continue;
    }
    ...
    logger.debug(
      `Registered session ${session.sessionId} (thread: ${session.rootMessageId})`
    );
  }
      ...
      if (TmuxBackend.hasSession(tmuxName)) {
        logger.info(
          `[${ds.session.sessionId.substring(
            0,
            8
          )}] Tmux session alive, auto-forking worker to re-attach`
        );
        forkWorker(ds, "", true);
      }
    }
  ...
  logger.info(
    `Restored ${active.length} session(s)${
      config.daemon.backendType === "tmux"
        ? ""
        : ", waiting for messages to resume"
    }`
  );
}
...
export async function executeScheduledTask(
  ...
  if (task.rootMessageId) {
    try {
      ...
      logger.warn(
        `[scheduler] Failed to reply in original thread ${task.rootMessageId} (${err.message}); falling back to new thread`
      );
      threadRootId = await sendMessage(
        larkAppId,
        task.chatId,
        `🕐 定时任务「${task.name}」开始执行`
      );
  ...
  if (isContinuation && existing?.worker && !existing.worker.killed) {
    ...
    try {
      ...
      logger.info(
        `[scheduler] Task "${task.name}" injected into live session ${existing.session.sessionId}`
      );
      return;
    } catch (err: any) {
      logger.warn(
        `[scheduler] Failed to inject into live session (${err.message}); spawning fresh worker`
      );
    }
  }
  ...
  logger.info(
    `[scheduler] Task "${task.name}" spawned (session: ${session.sessionId}, thread: ${threadRootId}, continuation: ${isContinuation})`
  );
}
