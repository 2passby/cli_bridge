...
<p align="center">
  ...
  <a href="https://github.com/deepcoldy/botbridge"><img src="https://img.shields.io/github/stars/deepcoldy/botbridge.svg?style=social" alt="GitHub Stars"></a>
</p>
...
botbridge 不重新实现 Agent 能力，而是直接桥接已有的 AI 编程 CLI（Claude Code、Codex、Gemini、OpenCode）。记忆、上下文管理、工具调用、权限体系，这些能力 CLI 本身都在快速迭代，botbridge 选择站在这个进化之上，而不是平行重造一套。CLI 的每次升级，botbridge 零适配自动受益。

### 核心优势

与 OpenClaw 等基于 Agent SDK 构建的方案相比：

| 特性 | botbridge | OpenClaw 类方案 |
|------|--------|----------------|
...
| 安装部署 | `npm install -g botbridge`，5 分钟飞书配置即可使用 | 安装简单，但配置项较多 |
...
**核心收益：Daemon 重启不中断 CLI。** `botbridge restart` 时 worker 进程退出，但 tmux session（及其中的 CLI 进程）保持运行。下次收到消息时 worker 自动 re-attach，无需 `--resume` 重载上下文。

| 事件 | tmux session | CLI 进程 |
|------|-------------|---------|
| `botbridge restart` | 存活 | 存活（下次消息 re-attach） |
...
# 也可以手动 attach（会话名 = bbg-<sessionId 前 8 位>）
tmux attach -t bbg-<session-id-前8位>
...
`botbridge list` 提供交互式 TUI，显示所有活跃会话的 ID、标题、工作目录、PID、运行时长和状态，方向键选择后回车即可 attach。也支持 `botbridge list --plain` 输出纯文本表格供脚本使用。

**tmux 会话命名规则：** `bbg-<sessionId 前 8 位>`

### 会话接入（Adopt）

将已在 tmux 中运行的 CLI 进程无缝接入 BotBridge，在手机上通过飞书查看进度和交互。
...
- **共享模式** — BotBridge 接入后，iTerm2 和飞书双向同步：流式卡片实时显示终端输出，飞书聊天框输入直接透传到终端
- **一键接管** — 点击流式卡片上的「🔄 接管」按钮，BotBridge 以 `--resume` 重建会话，转为标准 BotBridge 会话
- **安全断开** — 点击「⏏ 断开」，BotBridge 退出观察，原 CLI 不受影响
...
- **对话触发**（灵活）：直接跟 agent 说「帮我加个每天 18:00 检查部署的定时任务」，自动触发 `botbridge-schedule` Skill
...
CLI 进入 botbridge 会话时自动获得 `~/.botbridge/bin` 在 PATH 中，以及一组开箱即用的 Skill：

- `botbridge send` — 向当前话题发消息（支持文本、图片、文件、@mention）
- `botbridge thread messages` — 读取当前话题的历史消息
- `botbridge bots list` — 查询当前群聊的机器人及 open_id
- `botbridge schedule` — 增删改查定时任务
...
```bash
# 安装 npm 包（包名为 botbridge，CLI 命令仍为 botbridge）
...
1. **订阅方式**：点击编辑图标，选择「使用长连接接收事件」（需要 botbridge 已启动，飞书会检测长连接是否建立）
...
7. CLI 通过 `botbridge send` 命令在话题中回复（由 `botbridge-send` Skill 自动引导）
...
**推荐方式：跟 agent 对话**
直接说「每天 9:00 帮我生成昨天 PR 汇总」，agent 会用 `botbridge-schedule` Skill 处理并跟你确认。
...
| 命令 | 说明 |
|------|------|
| `botbridge setup` | 交互式配置（首次使用 / 添加机器人） |
| `botbridge start` | 启动 daemon（PM2 管理） |
| `botbridge stop` | 停止 daemon |
| `botbridge restart` | 重启 daemon（自动恢复活跃会话） |
| `botbridge logs` | 查看日志（`--lines N`） |
| `botbridge status` | 查看 daemon 状态 |
| `botbridge upgrade` | 升级到最新版本 |
| `botbridge list` | 列出所有活跃会话（别名 `ls`） |
| `botbridge delete <id>` | 关闭指定会话，支持 ID 前缀匹配（别名 `del`/`rm`） |
| `botbridge delete all` | 关闭所有活跃会话 |
| `botbridge delete stopped` | 清理所有进程已退出的僵尸会话 |
...
| 命令 | 说明 |
|------|------|
| `botbridge send [content]` | 向当前话题发消息。支持 stdin / heredoc / `--content-file` 传内容，`--images`/`--files`/`--mention` 附加资源 |
| `botbridge bots list` | 列出当前群聊中的机器人（含 open_id，供 `--mention` 使用） |
| `botbridge thread messages [--limit N]` | 拉取当前话题的消息历史（JSON） |
| `botbridge schedule add <schedule> <prompt>` | 创建定时任务（自动绑定当前话题） |
| `botbridge schedule list/remove/pause/resume/run` | 管理定时任务 |

这些命令依赖 `~/.botbridge/bin/botbridge` 这个 wrapper 脚本，daemon 启动时自动写入并加入 worker 的 PATH，版本始终与 daemon 一致（不需要 `npm i -g`）。

---

## 配置

通过 `~/.botbridge/bots.json` 配置机器人。运行 `botbridge setup` 交互式创建，或手动编辑。
...
**配置优先级：** `BOTS_CONFIG` 环境变量 → `~/.botbridge/bots.json`
...
| `SESSION_DATA_DIR` | `~/.botbridge/data` | 会话和队列的存储目录 |
...
| 路径 | 说明 |
|------|------|
| `~/.botbridge/bots.json` | 机器人配置文件 |
| `~/.botbridge/data/` | 会话数据、消息队列 |
| `~/.botbridge/logs/` | Daemon 日志 |
